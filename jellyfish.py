...
class JellyfishTopo(StructuredTopo):
  ...
  def not_fully_connected(self, switch_list, link_list):#bu shi quan lian tong
  ...
  def addHosts(self, switches, nodes, ports_per_switch, hosts_per_switch):
    ...
    if len(host_list):#if some hosts are left alone, say 13 hosts, 2 hosts per switch, the 13th host is going to be linked to the 7th switch
	switch_id = self.id_gen(switch_num, 255).name_str()
        switch_opts = self.def_nopts(self.LAYER_EDGE, switch_id)
        switch = self.addSwitch(switch_id, **switch_opts)
        lg.debug("Adding switch: %s\n" % (switch_id))
    ...
    for num in range(nodes / hosts_per_switch, switches):#if a switch has already been addded, what will addSwitch do?
  ...
  def switch_is_fully_connected(self, switch, links, switches):# switch has formed reciprocal connection with all other switches
    for s in switches:
      if s == switch:
        continue
  ...
  def create_topology(self, switches, nodes, ports_per_switch, hosts_per_switch):
    ...
    #SIMPLEST ISOMER TREE 
    #MAKE SURE GRAPH IS CONNECTED
    for num in range(0,len(switch_list)-1):
      switch1 = switch_list[num]
      switch2 = switch_list[num+1]
      link = (switch1, switch2)
      added_links.append(link)
    if ports_per_switch == 2:
      for num in range(1,len(switch_list)-1):
        switch_list.remove(switch_list[1])
    #print ("len(switch_list)=%d"%len(switch_list))
